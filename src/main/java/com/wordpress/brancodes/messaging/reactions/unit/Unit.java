...
public enum Unit {
	KG(MASS, n -> convert(n, 2.2046226218487) + " lbs", n -> n, n -> n),
	LB(MASS, n -> convert(n, 0.45359237) + " kgs", n -> convert(n, 0.45359237), n -> convert(n, 1 / 0.45359237)), // not actually mass, gravitational force on earth
	M(LENGTH, n -> convertedWithInches(n, 3.2808398950131), n -> n, n -> n),
	CM(LENGTH, n -> convertedWithInches(n, .0328083989501), n -> convert(n, .01), n -> convert(n, 100)),
	FT(LENGTH, n -> convert(n, 0.3048) + " m", n -> convert(n, 0.3048), n -> convert(n, 1 / 0.3048)),
	IN(LENGTH, n -> convert(n, 2.54) + " cm", n -> convert(n, .0254), n -> convert(n, 1 / .0254));
	...
	Unit(BaseUnitType baseUnitType, Function<ScaledDecimal, String> converter, Function<ScaledDecimal, ScaledDecimal> normalizer, Function<ScaledDecimal, ScaledDecimal> fromNormal) {
	...
	private static String convertedWithInches(ScaledDecimal input, double scale) {
		...
		if (feet.getFull().equals(BigDecimal.ZERO))
			return inches + "\"";
		if (inches.getFull().equals(BigDecimal.ZERO))
			return feet + "'";
		return feet + "'" + inches + "\"";
	}
